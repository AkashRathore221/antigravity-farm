...
const agBelts = [
  { name: 'Bangalore Polyhouse Belt (India)', lat: 12.9716, lon: 77.5946 },
  { name: 'Nashik Greenhouse Zone (India)', lat: 19.9975, lon: 73.7898 },
  { name: 'Pune Horticulture Zone (India)', lat: 18.5204, lon: 73.8567 },
  { name: 'Delhi NCR Agri Belt (India)', lat: 28.6139, lon: 77.2090 },
];
...
export const Weather: React.FC = () => {
  ...
  const [locationName, setLocationName] = useState('Bangalore Polyhouse Belt (India)');
  const [activeCoords, setActiveCoords] = useState({ lat: 12.9716, lon: 77.5946 });
  ...
  const captureGpsLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        alert('Failed to capture GPS. Falling back to manual zone selection.');
        setGpsLoading(false);
      }
    );
  };

  const handleManualLocationChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    const belt = agBelts.find(b => b.name === e.target.value);
    if (belt) {
      setLocationName(belt.name);
      setActiveCoords({ lat: belt.lat, lon: belt.lon });
      setForecast([]);
    }
  };
  ...
  const coordsLabel = agBelts.find(b => b.name === locationName)
    ? `${activeCoords.lat}° N, ${activeCoords.lon}° E`
    : locationName;
      ...
      <div className="glass rounded-2xl p-5 border border-slate-200/30 dark:border-slate-800/30 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          ...
          <div className="space-y-0.5">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block">Active Micro-climate Belt</span>
            <h4 className="text-lg font-black text-slate-850 dark:text-slate-100 font-heading">{locationName}</h4>
            <span className="text-xs text-slate-400 font-bold">{coordsLabel}</span>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <select
            ...
            value={locationName}
            className="text-xs bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl px-3 py-2.5 font-semibold text-slate-600 dark:text-slate-300 focus:outline-none"
          >
            {agBelts.map(b => (
          ...
          <button
            onClick={() => fetchWeather(activeCoords.lat, activeCoords.lon)}
            disabled={apiLoading}
            className="px-4 py-2.5 bg-sky-500/10 hover:bg-sky-500/20 border border-sky-500/20 hover:border-sky-500/30 text-sky-600 dark:text-sky-400 text-xs font-bold rounded-xl flex items-center gap-1.5 transition-all shadow-sm shrink-0"
          >
            <RefreshCw size={14} className={apiLoading ? 'animate-spin' : ''} />
            {apiLoading ? 'Fetching...' : 'Fetch Live Weather'}
          </button>
